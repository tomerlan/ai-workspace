...
Temperature compensation holds the period nearly constant across the physiological temperature range, although reaction rates in the underlying biochemistry vary with temperature in the ordinary way. The cancellation that produces this is described in 2.6.
...
**The state variables.** These are the quantities whose values are the clock's position: the abundance of *Per* and *Cry* transcripts, the cytoplasmic and nuclear concentrations of their protein products, the phosphorylation state of PER, and the occupancy of CLOCK:BMAL1 at E-box elements. *Bmal1* cycles in antiphase to *Per*, driven by the REV-ERB and ROR arm acting on its promoter. Remove *Per* or *Cry* and timekeeping stops.

In cultured cells the trajectory is followed continuously as bioluminescence from a PER2::LUCIFERASE fusion, which reports one protein's abundance as a proxy for the whole state.

**What the loop drives.** Everything else that cycles in a cell does so because CLOCK:BMAL1 binds E-boxes in its promoter and the REV-ERB/ROR arm gates it. These transcripts carry the clock's timing outward without participating in timekeeping: remove any one and the clock still runs.

In a single organ roughly 5–15% of expressed transcripts cycle. Across twelve mouse organs, 43% of all protein-coding genes are rhythmic in at least one tissue, and apart from the core clock genes themselves the rhythmic sets are largely non-overlapping between organs (Zhang et al., 2014). The same timing mechanism therefore runs almost entirely different output programs in liver, adipose, and muscle. Rhythms in protein and metabolite abundance are only partly predicted by transcript rhythms, a large fraction arising post-transcriptionally. Within an organ the rhythmic transcripts do not all peak together but cluster into successive waves, each corresponding to a coordinated block of the tissue's function.

**The coupled oscillator.** Redox poise across the shared carrier pools cycles as well, and it is not an output of the loop. It is a second oscillator, running in cells with no transcription at all, coupled to the transcriptional one in both directions (2.7, 2.8, 2.9).

**At the level of the organism** the measurable quantities are plasma hormone concentrations, core body temperature, blood pressure, sleep propensity, cognitive throughput, and the rates of secretion, absorption, and excretion. All of these are driven outputs, which is why a phase read from any one of them is an inference about the pacemaker rather than a direct observation of it.
...
**Nutrient state.** Insulin, glucose, and the NAD⁺/SIRT1 and AMPK pathways report feeding at the level of cellular metabolism rather than as a behavioural event (5.9).
...
**Damping the clock opens the cell cycle gate.** PER1 and PER2 couple the clock to mitosis through E-box control of Cyclin D1, Cyclin A, Mdm-2, Gadd45α and c-MYC, and *Per2*-null animals show increased tumour formation with decreased apoptosis (7.1). Damping clock gene cycling to slow the protein economy removes that gating as a side effect, and proliferation proceeds without the phase restriction that confines it. Melatonin amplitude and repair timing (7.2, 7.3) fail from the same light input, so all three routes open together. The coupling itself is established; the route to it through the protein economy is his.
...
Measure: all five, separately, per 11.1 and 11.3. Each is defined in 9.2.1 to 9.2.5.
...
- **2011** — sections 5, 7
- **2012** — sections 4, 5, 6, 11
- **2013** — sections 2
- **2015** — sections 2, 4, 5, 6, 7, 11
- **2016** — sections 4, 5, 7, 9, 10, 11
- **2018** — sections 2, 4, 5, 6, 11
- **2019** — sections 4, 5, 7, 11
- **2022** — sections 4, 5, 9, 10, 11
- **2023** — sections 2
- **2025** — sections 11
